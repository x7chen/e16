...
#define BLE_UUID_EBIKE_CHARACTERISTIC_BATTERY		0x1502
#define BLE_UUID_EBIKE_CHARACTERISTIC_SENSOR_DATA	0x1503
#define BLE_UUID_EBIKE_CHARACTERISTIC_SPECIAL		0x1504
#define BLE_UUID_EBIKE_CHARACTERISTIC_SETTINGS		0x1505
#define BLE_UUID_EBIKE_CHARACTERISTIC_ERROR			0x1506
#define BLE_UUID_EBIKE_CHARACTERISTIC_IDENTIFIERS	0x1507
#define BLE_UUID_EBIKE_CHARACTERISTIC_SERIAL_NUMBER	0x1508
#define BLE_UUID_EBIKE_CHARACTERISTIC_FEEDBACK		0x1509
...
static uint32_t battery_add_characteristic_add(ble_ebike_t * p_ebike, const ble_ebike_init_t * p_ebike_init)
{
...
static uint32_t special_characteristic_add(ble_ebike_t * p_ebike, const ble_ebike_init_t * p_ebike_init)
{
	/**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    ble_uuid.type = p_ebike->uuid_type;
    ble_uuid.uuid = BLE_UUID_EBIKE_CHARACTERISTIC_SPECIAL;
    ...
    return sd_ble_gatts_characteristic_add(p_ebike->service_handle,
                                           &char_md,
                                           &attr_char_value,
                                           &p_ebike->special_handles);
    /**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
}



static uint32_t settings_characteristic_add(ble_ebike_t * p_ebike, const ble_ebike_init_t * p_ebike_init)
{
	/**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    ble_uuid.type = p_ebike->uuid_type;
    ble_uuid.uuid = BLE_UUID_EBIKE_CHARACTERISTIC_SETTINGS;
...
static uint32_t identifiers_characteristic_add(ble_ebike_t * p_ebike, const ble_ebike_init_t * p_ebike_init)
{
	/**@snippet [Adding proprietary characteristic to S110 SoftDevice] */
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_md_t cccd_md;
    ...
    ble_uuid.type = p_ebike->uuid_type;
    ble_uuid.uuid = BLE_UUID_EBIKE_CHARACTERISTIC_IDENTIFIERS;
...
//    }
	...
	err_code = battery_add_characteristic_add(p_ebike, p_ebike_init);
	if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
	...
	err_code = special_characteristic_add(p_ebike, p_ebike_init);
	if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
	err_code = settings_characteristic_add(p_ebike, p_ebike_init);
	if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
	...
	err_code = identifiers_characteristic_add(p_ebike, p_ebike_init);
	if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
uint32_t ble_ebike_special_update(ble_ebike_t * p_ebike, uint8_t * p_dat, uint16_t length)
{
	...
	// Update database.
	err_code = sd_ble_gatts_value_set(p_ebike->conn_handle,
									  p_ebike->special_handles.value_handle,
									  &gatts_value);
	if (err_code != NRF_SUCCESS)
	{
		return err_code;
	...
	if ((p_ebike->conn_handle != BLE_CONN_HANDLE_INVALID) && p_ebike->is_notification_enabled)
	{
		...
		hvx_params.handle = p_ebike->special_handles.value_handle;
		hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
		hvx_params.offset = gatts_value.offset;
		hvx_params.p_len  = &gatts_value.len;
		hvx_params.p_data = gatts_value.p_value;
...
uint32_t ble_ebike_settings_update(ble_ebike_t * p_ebike, uint8_t * p_dat, uint16_t length)
{
	...
	// Update database.
	err_code = sd_ble_gatts_value_set(p_ebike->conn_handle,
									  p_ebike->settings_handles.value_handle,
									  &gatts_value);
	if (err_code != NRF_SUCCESS)
	{
		return err_code;
	...
	if ((p_ebike->conn_handle != BLE_CONN_HANDLE_INVALID) && p_ebike->is_notification_enabled)
	{
		...
		hvx_params.handle = p_ebike->settings_handles.value_handle;
		hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
		hvx_params.offset = gatts_value.offset;
		hvx_params.p_len  = &gatts_value.len;
		hvx_params.p_data = gatts_value.p_value;
...
uint32_t ble_ebike_identifiers_update(ble_ebike_t * p_ebike, uint8_t * p_dat, uint16_t length)
{
	...
	// Update database.
	err_code = sd_ble_gatts_value_set(p_ebike->conn_handle,
									  p_ebike->identifiers_handles.value_handle,
									  &gatts_value);
	if (err_code != NRF_SUCCESS)
	{
		return err_code;
	...
	if ((p_ebike->conn_handle != BLE_CONN_HANDLE_INVALID) && p_ebike->is_notification_enabled)
	{
		...
		hvx_params.handle = p_ebike->identifiers_handles.value_handle;
		hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
		hvx_params.offset = gatts_value.offset;
		hvx_params.p_len  = &gatts_value.len;
		hvx_params.p_data = gatts_value.p_value;
